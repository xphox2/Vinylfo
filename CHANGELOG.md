...
#### Feed Connection + Preview Reliability

- Fixed Settings → Feeds preview iframes opening SSE connections on page load by defaulting previews to `about:blank` and only loading previews when a sample track is selected.
- Fixed demo/preview feeds (`demoTrack`) unnecessarily opening SSE connections by skipping SSE entirely in demo mode.
- Reduced SSE lock contention and per-connect database load in the shared video-feed SSE endpoint (client snapshot broadcast, cached track info, bounded initial-state send, heartbeat keepalive).
- Fixed Track Feed marquee not scrolling for short text by always animating at the configured speed/direction (build repeated segments and animate by pixel distance).
...
- `controllers/video_feed.go` - Hardened shared SSE endpoint (heartbeat, cached initial state, lower lock contention)
- `templates/settings-feeds.html` - Default preview iframes to `about:blank` to prevent auto-connecting
- `static/js/settings-feeds.js` - Only load preview if a sample track is selected
- `static/js/album-art-feed.js` - Skip SSE in demo mode (preview)
- `templates/track-feed.html` - Marquee content generated by JS for repeatable scrolling
- `static/css/track-feed.css` - Pixel-based marquee animation with correct direction handling
- `static/js/track-feed.js` - Always-on marquee scrolling at configured speed/direction; skip SSE in demo mode
...
#### Player Multi-Tab Sync

- Fixed `/player` not staying in sync across multiple tabs by adding real-time Server-Sent Events (SSE) updates for playback state/position (with polling fallback).
- New endpoint: `GET /playback/events`
- Files: `controllers/playback.go`, `routes/routes.go`, `static/js/playback-dashboard.js`

## [0.3.5-alpha] - 2026-01-29

### Fixed

#### UI Cleanup

- Fixed Delete button not working on the YouTube playlists page
- Fixed playlists to fail loading when quickly navigating back from playlist details
- Cleaned up Playlist options. Moved them to the main playlist screen instead of within the playlist itself.
